...
├── sudoku/
│   ├── engine.go            # Board type, PlaceNumber, ClearCell, IsSolved, etc.
...
└── ui/
    ├── app.go               # Fyne app setup, toolbar, numpad, timer, dark mode
...
./deploy_android.sh
```
Builds an APK via `fyne package --target android` and installs via `adb`.
